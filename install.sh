...
# --- args ---
TARGET=""; STACK=""; COMPONENTS=""; SCOPE="project"; DO_PLUGIN=1; DO_GLOBAL=""; ASSUME_YES=""
while [ "$#" -gt 0 ]; do
  case "$1" in
...
if [ "$DO_PLUGIN" = 1 ]; then
  ...
  python3 - "$settings" "$MKT_NAME" "$MKT_REPO" <<'PY'
import json, os, sys
p, mkt, repo = sys.argv[1:4]
s = json.load(open(p)) if os.path.exists(p) and os.path.getsize(p) > 0 else {}
s.setdefault("extraKnownMarketplaces", {}).setdefault(mkt, {"source": {"source": "github", "repo": repo}})
s.setdefault("enabledPlugins", {})[f"{mkt}@{mkt}"] = True
with open(p, "w") as f:
    json.dump(s, f, indent=2); f.write("\n")
print(f"  enabled plugin {mkt}@{mkt} in {os.path.basename(p)}")
PY
else
...
✔ harness wired into $TARGET
  ...
  plugin      : $([ "$DO_PLUGIN" = 1 ] && echo "enabled ($SCOPE scope)" || echo "not enabled")
