<!-- guardrails v1 | RUNTIME playbook | adapted from github.com/TheColliny/FableClaudeMDForOpus -->
...
- R5. Source of truth (configure for your shop): your APM's per-action throughput (Performance -> Actions/Transactions for your prod app); your org's usage/cost dashboard for LLM/API/infra spend; the background-job store or queue metrics for job-run counts. Prefer these over inference from the code.

--- reference ---

## The recurring failure this prevents
Tracing a code path and asserting it as a live behavior/problem without confirming execution. Typical instances: a bug-location premise asserted from code before tracing evidence; a "warm-cache job erroring in prod" claimed from code before the APM record confirmed it; a "double-processing" cost spike scoped around an ingestion path that carried ZERO prod traffic (0 executions over 30 days) while everything actually ran through a different, high-volume path. Code shows structure; runtime shows reality.
