<!-- guardrails v1 | CODE playbook | adapted from github.com/TheColliny/FableClaudeMDForOpus -->
...
- C2. Generated/vendored check: path contains `dist/`, `build/`, `out/`, `node_modules/`, `vendor/`, `target/`, `tmp/`, `coverage/`, `*.min.*`, or is a lockfile or generated schema dump (e.g. `package-lock.json`, `yarn.lock`, `pnpm-lock.yaml`, `Gemfile.lock`, `composer.lock`, `go.sum`, `Cargo.lock`, `db/schema.rb`, `db/structure.sql`) — or the first 10 lines say "DO NOT EDIT"/"@generated"? Do not edit -> change the source or generator (the migration for a schema dump, the manifest for a lockfile), re-run it, and name the generator command.
- C3. Twin check: Grep the target file/symbol name repo-wide before the first edit. More than one definition? List all candidates and paste the evidence for the live one (the import/require line or stack frame pointing at it). Common generic names (`create`, `update`, `show`, `handle`, `call`, `index`) match many — this fires often.
...
- C7. Touching any category in `${CLAUDE_PLUGIN_ROOT}/skills/guardrails/references/TRAPS.md` — dates/times, epochs/units, mutation-vs-copy, async, floats/money, sort, division/modulo, regex, familiar-API traps, closures, boolean logic? Read TRAPS.md and follow your rows — never guess load-bearing behavior.
...
When your global CLAUDE.md routes this edit to an implementation sub-agent, the sub-agent does not automatically read this file. Either perform the C1/C12 gates yourself before delegating, or include "read ${CLAUDE_PLUGIN_ROOT}/skills/guardrails/references/CODE.md and follow C1, C12/REFERENCE SWEEP" in the sub-agent's prompt.
