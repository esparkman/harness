<!-- guardrails v1 | MECHANISM playbook | adapted from github.com/TheColliny/FableClaudeMDForOpus -->
...
A `WarmParserCacheJob` had a comment: "keep the schema cache warm… cached for 24 hours." I pattern-matched "cache" to **prompt caching**, read the prompt-caching docs (5-min/1h TTL, token floor), and concluded the comment's "24h" was fiction and the job was dead code — then built a delete + investigation + cost-spike + PR + issue-tracker moves on it. The author corrected it: the job warms the **structured-output grammar cache**, which the vendor's structured-outputs docs document as "cached for 24 hours from last use," with a compile-latency hit on a cold schema. The comment was accurate; I never read the doc for the actual mechanism. One M1 check (verify the feature the comment named against its own doc) would have caught it before any of the edifice was built.
